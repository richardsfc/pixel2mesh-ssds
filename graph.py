...
class Graph:
    ...
    def to_obj(self, obj_file):
        ...
        for id_vertex in range(vertices_copy.shape[0]):
            ...
            xcoord = str(vertices_copy[id_vertex, 0])
            ycoord = str(vertices_copy[id_vertex, 1])
            zcoord = str(vertices_copy[id_vertex, 2])
...
if __name__ == "__main__":
    ...
    graph.to_obj("./ellipsoid/ellipsoid_block1.obj")
    print("Vertices dim (block 1):", graph.vertices.size())
    graph.unpool()
    graph.to_obj("./ellipsoid/ellipsoid_block2.obj")
    print("Vertices dim (block 2):", graph.vertices.size())
    graph.unpool()
    graph.to_obj("./ellipsoid/ellipsoid_block3.obj")
